...
void sendInfoToHost(u8 cmd)
{
	...
	len = 0;
	needCRC = 1;
	inforToHost[len] = UART_TRANSCTL;len++;
	switch (cmd)
	{
		case CMD_SAME_ID:
			inforToHost[len] = CMD_SAME_ID;len++;
			memcpy(inforToHost+len,gb_lastCmdId,sizeof(gb_lastCmdId));
			len += 3;
		  needCRC = 0;
		break;
		case CMD_HANDSHAKE:
			inforToHost[len] = CMD_HANDSHAKE;len++;
			memcpy(inforToHost+len,gb_lastCmdId,sizeof(gb_lastCmdId));
			len += 3;
		  needCRC = 0;
		break;
		case CMD_INIT:
			inforToHost[len] = UART_FREAM_START;len++;
		  inforToHost[len] = 0x00;len++;
		  inforToHost[len] = 0x08;len++;
			inforToHost[len] = CMD_INIT;len++;
		  for(i=0; i<4; i++)
			{
				inforToHost[len] = gb_status[i];len++;
			}
			inforToHost[len] = gb_cmdExecutionResult;len++;
			inforToHost[len] = LowByte(gb_cmdExecutionErrCode);len++;
			inforToHost[len] = HighByte(gb_cmdExecutionErrCode);len++;
	
			memcpy(inforToHost+len,gb_lastCmdId,sizeof(gb_lastCmdId));len += 3;
			inforToHost[len] = UART_TRANSCTL;len++;
			inforToHost[len] = UART_FREAM_END;len++;
		break;	
		case CMD_BILL_ENABLE:
			inforToHost[len] = UART_FREAM_START;len++;
		  inforToHost[len] = 0x00;len++;
		  inforToHost[len] = 0x08;len++;
			inforToHost[len] = CMD_BILL_ENABLE;len++;
		  for(i=0; i<4; i++)
			{
				inforToHost[len] = gb_status[i];len++;
			}
		...
		case CMD_BILL_DISABLE:
		  ...
		  inforToHost[len] = 0x00;len++;
		  inforToHost[len] = 0x08;len++;
			inforToHost[len] = CMD_BILL_DISABLE;len++;
		  for(i=0; i<4; i++)
			{
				inforToHost[len] = gb_status[i];len++;
			}
		...
		case CMD_READ_STATUS:
		  ...
		  inforToHost[len] = 0x00;len++;
		  inforToHost[len] = 0x08;len++;
			inforToHost[len] = CMD_READ_STATUS;len++;
		  for(i=0; i<4; i++)
			{
				inforToHost[len] = gb_status[i];len++;
			}
			...
			inforToHost[len] = LowByte(gb_cmdExecutionErrCode);len++;
			inforToHost[len] = HighByte(gb_cmdExecutionErrCode);len++;
			inforToHost[len] = gb_billState;len++;
			inforToHost[len] = gb_CurrentValue;len++;
			inforToHost[len] = gb_CurrentHaveBill;len++;
		...
		case CMD_READ_VERSION:
			inforToHost[len] = UART_FREAM_START;len++;
		  inforToHost[len] = 0x00;len++;
		  inforToHost[len] = 0x15;len++;
			inforToHost[len] = CMD_READ_VERSION;len++;
		  for(i=0; i<4; i++)
			{
				inforToHost[len] = gb_status[i];len++;
			}
			...
			inforToHost[len] = LowByte(gb_cmdExecutionErrCode);len++;
			inforToHost[len] = HighByte(gb_cmdExecutionErrCode);len++;
			memcpy(inforToHost+len, USER1_VERSION,sizeof(USER1_VERSION));len += 11;
			memcpy(inforToHost+len, USER2_VERSION,sizeof(USER2_VERSION));len += 11;
		...
		case CMD_CLEAR_TD:
		  ...
		  inforToHost[len] = 0x00;len++;
		  inforToHost[len] = 0x15;len++;
			inforToHost[len] = CMD_CLEAR_TD;len++;
		  for(i=0; i<4; i++)
			{
				inforToHost[len] = gb_status[i];len++;
			}
	...
	if(len > 0)
	{
		if(needCRC == 1)
		{
			crcRes = crcVal(inforToHost,len);
			inforToHost[len]= (u8)crcRes;len++;
			inforToHost[len]= (u8)(crcRes>>8);len++;
		}
		if(gb_uart1Mode == TRANSFER_MODE)
		{
			uart_SendDataToUart1(inforToHost, len);
#ifdef UART_DEBUG
...
void UserCopyPac(UART_CMD UserPac)
{
	...
	u8 dataLen;
	len = 0;
	gb_bkPacage[len++] = UserPac.transmissionCtr1;
	gb_bkPacage[len++] = UserPac.dataStart;
	gb_bkPacage[len++] = UserPac.dataLen[0];
	gb_bkPacage[len++] = UserPac.dataLen[1];
	gb_bkPacage[len++] = UserPac.CMD1;
	switch(UserPac.CMD1)
	{
		case CMD_INIT:
		case CMD_BILL_ENABLE:
	...
	for(i=0; i<dataLen; i++)
	{
		gb_bkPacage[len++] = UserPac.DATA[i];
	}
	for(i=0; i<3; i++)
	{
		gb_bkPacage[len++] = UserPac.CMDid[i];
	}
	gb_bkPacage[len++] = UserPac.transmissionCtr2;
	gb_bkPacage[len++] = UserPac.dataEnd;
	gb_bkPacageLen = len;
}
UART_CMD m_bkPacage;
void DealBKPackage(void)
{
	...
	if (uart1Pacinfifo_count)
	{
		m_bkPacage = uart1Pacinfifo_DataOut();
		if(m_bkPacage.CMDid[0] == gb_lastCmdId[0]
			&&m_bkPacage.CMDid[1] == gb_lastCmdId[1]
		  &&m_bkPacage.CMDid[2] == gb_lastCmdId[2])
		{
			//??1015+ID
			sendInfoToHost(CMD_SAME_ID);
		}
		else
		{
			  if(gb_RequestContent == 1)
				{
					gb_lastCmdId[0] = m_bkPacage.CMDid[0];
					gb_lastCmdId[1] = m_bkPacage.CMDid[1];
					gb_lastCmdId[2] = m_bkPacage.CMDid[2];
//					switch (gb_lastCmd)
//					{
//						case CMD_INIT:			 
//						//?????? 
//						sendInfoToHost(CMD_INIT);
//						break;
//						case CMD_READ_STATUS:
						sendInfoToHost(gb_lastCmd);
//						break;
//						default:
//							break;
//						}
				...
				{
					UserCopyPac(m_bkPacage);
					p_package = gb_bkPacage;
					m_packageLen =  gb_bkPacageLen;
					crcRes = crcVal(p_package,m_packageLen);
					m_crcRes[0] = (u8)crcRes;
					m_crcRes[1] = (u8)(crcRes>>8);
#ifdef UART_DEBUG						
						uart_SendDataToUart3(m_crcRes,2);
#endif
					if(m_bkPacage.CrcData[0] == m_crcRes[0]&&m_bkPacage.CrcData[1] == m_crcRes[1])
					{
						gb_lastCmdId[0] = m_bkPacage.CMDid[0];
						gb_lastCmdId[1] = m_bkPacage.CMDid[1];
						gb_lastCmdId[2] = m_bkPacage.CMDid[2];
#ifdef UART_DEBUG						
						uart_SendDataToUart3(gb_lastCmdId,3);
#endif
						switch (m_bkPacage.CMD1)
						{
							case CMD_INIT:			 
								gb_lastCmd = CMD_INIT;
								//??????
					...
					{
						//????
						gb_uartPacErrInfo = UART_PAC_ERR_CRC;
					}
			 }
			}
		}
...
void uartPacProcess(u8 data)
{
	static u8 m_pacLen;
	static u8 m_pacLenCnt;
	switch (gb_uartPacDealStep)
		{
		case UART_IDEL:
			if (data == UART_TRANSCTL)
			{
				gb_packageFromUart1.transmissionCtr1 = data;
				gb_uartPacDealOverTime = uartPacDealMaxTime;
				gb_uartPacDealStep = UART_START;
//#ifdef UART_DEBUG
//		uart_SendDataToUart3(&gb_uartPacDealStep,1);
//#endif
			}
			else
			{
				gb_uartPacErrInfo = UART_PAC_ERR_CTRL1+gb_uartPacDealStep;
				gb_uartPacDealOverTime = 0;
			}
		break;
		case UART_START:
			if (data == UART_FREAM_START)
			{
				gb_packageFromUart1.dataStart = data;
				gb_uartPacDealOverTime = uartPacDealMaxTime;
				gb_uartPacDealStep = UART_PAC_LEN;
				m_pacLenCnt = 0;
				gb_RequestContent = 0;
//				#ifdef UART_DEBUG
//		uart_SendDataToUart3(&gb_uartPacDealStep,1);
//#endif
			...
			else if(data == UART_FREAM_REQUEST)//????
			{
				gb_uartPacDealStep = UART_PAC_ID;
				m_pacLen = 3;
				m_pacLenCnt = 0;
				gb_RequestContent = 1;
			}
			...
			{
				gb_uartPacErrInfo = UART_PAC_ERR_START+gb_uartPacDealStep;
				gb_uartPacDealOverTime = 0;
			}
		break;	
		case UART_PAC_LEN:
				gb_packageFromUart1.dataLen[m_pacLenCnt] = data;
				gb_uartPacDealOverTime = uartPacDealMaxTime;
		    m_pacLenCnt++;
        if(m_pacLenCnt == 2)
				{
					gb_uartPacDealStep = UART_PAC_CMD;
				}
				else
				{
					gb_uartPacDealStep = UART_PAC_LEN;
...
//#endif
		break;
		case UART_PAC_CMD:
			gb_packageFromUart1.CMD1 = data;
		  gb_uartPacDealOverTime = uartPacDealMaxTime;
		  gb_uartPacDealStep = UART_PAC_DATA;
			switch(gb_packageFromUart1.CMD1)
			{
				case CMD_INIT:
				case CMD_BILL_ENABLE:
				case CMD_BILL_DISABLE:
...
//#endif
		break;
		case UART_PAC_DATA:
			gb_packageFromUart1.DATA[m_pacLenCnt++] = data;
		  gb_uartPacDealOverTime = uartPacDealMaxTime;
		  if(m_pacLenCnt == m_pacLen)
			{
				gb_uartPacDealStep = UART_PAC_ID;
				m_pacLen = 3;
				m_pacLenCnt = 0;
...
//#endif
		break;
		case UART_PAC_ID:
			gb_packageFromUart1.CMDid[m_pacLenCnt++] = data;
		  gb_uartPacDealOverTime = uartPacDealMaxTime;
		  if(m_pacLenCnt == m_pacLen)
			{
				if(gb_RequestContent == 1)
...
//#ifdef UART_DEBUG
//				uart_SendDataToUart3("456", 3);
//#endif
				}
				else
				{
					gb_uartPacDealStep = UART_PAC_CTRL;
				}
			}
//			#ifdef UART_DEBUG
//		uart_SendDataToUart3(&gb_uartPacDealStep,1);
//#endif
		...
		case UART_PAC_CTRL:
			if (data == 0x10)
			{
				gb_packageFromUart1.transmissionCtr2 = data;
				gb_uartPacDealOverTime = uartPacDealMaxTime;
				gb_uartPacDealStep = UART_PAC_END;
			}
			else
			{
				gb_uartPacErrInfo = UART_PAC_ERR_CTRL2+gb_uartPacDealStep;
				gb_uartPacDealStep = UART_IDEL;
				gb_uartPacDealOverTime = 0;
			}
//			#ifdef UART_DEBUG
...
//#endif
		break;
		case UART_PAC_END:
			if (data == UART_FREAM_END)
			{
				gb_packageFromUart1.dataEnd = data;
				gb_uartPacDealOverTime = uartPacDealMaxTime;
				gb_uartPacDealStep = UART_PAC_CRC;
				m_pacLen = 2;
				m_pacLenCnt = 0;
			}
			else
			{
				gb_uartPacErrInfo = UART_PAC_ERR_END+gb_uartPacDealStep;
				gb_uartPacDealStep = UART_IDEL;
				gb_uartPacDealOverTime = 0;
			}
//			#ifdef UART_DEBUG
...
//#endif
		break;
		case UART_PAC_CRC:
			gb_packageFromUart1.CrcData[m_pacLenCnt++] = data;
			if(m_pacLenCnt == m_pacLen)
			{
				gb_uartPacDealOverTime = 0;
				gb_uartPacDealStep = UART_IDEL;
...
{
	disp_clearScreen(BLACK);
	disp_DrawPic(100,60,BMP_BFLAGTRY);
}
...
void GetOpenCurrency(void)
{
	...
	{
		g_currency = INDEX_CNY;//openCurrency[0];
	}
}
...
//	TIM_TimeBaseInitTypeDef  TIM_TimeBaseStructure;
	...
	//更新面额
	GetOpenCurrency();
	g_currency = INDEX_CNY;//savedPara.machineWorkPara.d[DEFAULT_CURRENCY];
...
//					gb_billState = BILL_READY;
	...
	if(gb_uartPacDealOverTime > 0)
	{
		gb_uartPacDealOverTime--;
		if(gb_uartPacDealOverTime == 0)
		{
...
void USART1_IRQHandler(void)
{
	...
	if (USART_GetFlagStatus(USART1, USART_FLAG_RXNE))
	{
		...
		d = USART_ReceiveData(USART1);	//自动清除RXNE标志

		if(gb_uart1Mode == TRANSFER_MODE)
		{
#ifdef UART_DEBUG
		uart_SendDataToUart3(&d,1);
#endif
			uartPacProcess(d);
		}
