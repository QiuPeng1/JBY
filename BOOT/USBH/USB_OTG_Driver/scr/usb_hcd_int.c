...
uint32_t USBH_OTG_ISR_Handler (USB_OTG_CORE_HANDLE *pdev)
{
  ...
  if (USB_OTG_IsHostMode(pdev))
  {
    ...
    if (gintsts.b.sofintr)//在主机模式下，控制器设置此位指示有一个SOF(全速设备)或保持有效(低速设备)信号已发向USB总线。应用程序必需向此位写’1’来清除此中断。

    {
      retval |= USB_OTG_USBH_handle_sof_ISR (pdev);
    }
    
    if (gintsts.b.rxstsqlvl) // 接收FIFO非空 (RxFIFO non-empty)指示接收FIFO中至少有个一个数据包等待处理。

    {
      retval |= USB_OTG_USBH_handle_rx_qlvl_ISR (pdev);
    }
    
    if (gintsts.b.nptxfempty)  //非周期性发送FIFO已经全空,当非周期性发送FIFO全空或者半空，并且非周期性请求队列里至少能写入一个请求时，控制器会产生此中断。

    {
      retval |= USB_OTG_USBH_handle_nptxfempty_ISR (pdev);
    }
    
    if (gintsts.b.ptxfempty) //周期性发送FIFO已经全空 ,当周期性发送FIFO半空或者全空，并且周期性请求队列里至少能写入一个请求时，产生此中断。
    {
      retval |= USB_OTG_USBH_handle_ptxfempty_ISR (pdev);
    }    
    
    if (gintsts.b.hcintr)//主机通道中断 (Host channels interrupt)控制器设置此位，指示有一个未处理的主机通道事件(在主机模式下)。

    {
      retval |= USB_OTG_USBH_handle_hc_ISR (pdev);
    }
    
    if (gintsts.b.portintr)//主机端口中断 (Host port interrupt)控制器设置此位指示OTG_FS控制器的某个端口状态发生改变。

    {
      retval |= USB_OTG_USBH_handle_port_ISR (pdev);
    }
    
    if (gintsts.b.disconnect)//检测到断开事件中断 (Disconnect detected interrupt)当检测到设备断开时即产生此中断。
    ...
    if (gintsts.b.incomplisoout)//未完成的同步OUT传输 (Incomplete isochronous OUT transfer)在主机模式下，控制器在帧结束时仍有属于当前帧的未完成的周期性传输时，产生此中断。
...
*/
/*
主机通道中断 (Host channels interrupt)控制器设置此位，指示有一个未处理的主机通道事件(在主机模式下)。
应用程序需要读主机所有
通道中断寄存器(OTG_FS_HAINT寄存器)来获得产生中断的通道号，然后读相应的主机通道x中
断寄存器(OTG_FS_HCINTx寄存器)获得中断的具体信息。应用程序需要通过清除OTG_FS_HCINTx寄存器的相应位来清除此位。
*/
static uint32_t USB_OTG_USBH_handle_hc_ISR (USB_OTG_CORE_HANDLE *pdev)
{
...
*/
/*
在主机模式下，控制器设置此位指示有一个SOF(全速设备)或保持有效(低速设备)信号已发向USB总线。应用程序必需向此位写’1’来清除此中断。
*/
static uint32_t USB_OTG_USBH_handle_sof_ISR (USB_OTG_CORE_HANDLE *pdev)
{
  USB_OTG_GINTSTS_TypeDef      gintsts;
...
*/
/*
检测到断开事件中断 (Disconnect detected interrupt)当检测到设备断开时即产生此中断。
*/
static uint32_t USB_OTG_USBH_handle_Disconnect_ISR (USB_OTG_CORE_HANDLE *pdev)
{
  USB_OTG_GINTSTS_TypeDef      gintsts;
...
*/
/*
非周期性发送FIFO已经全空,当非周期性发送FIFO全空或者半空，并且非周期性请求队列里至少能写入一个请求时，控制器会产生此中断。
FIFO半空或全空的状态取决于控制器AHB配置寄存器的非周期性发送FIFO空级别位(OTG_FS_GAHBCFG寄存器的TXFELVL位)。
*/
static uint32_t USB_OTG_USBH_handle_nptxfempty_ISR (USB_OTG_CORE_HANDLE *pdev)
{
...
*/
/*
周期性发送FIFO已经全空 ,当周期性发送FIFO半空或者全空，并且周期性请求队列里至少能写入一个请求时，产生此中断。
半空还是全空的状态由控制器AHB配置寄存器的周期性发送FIFO空级别位决定(OTG_FS_GAHBCFG寄存器的PTXFELVL位)。
*/
static uint32_t USB_OTG_USBH_handle_ptxfempty_ISR (USB_OTG_CORE_HANDLE *pdev)
{
  USB_OTG_GINTMSK_TypeDef      intmsk;
...
#endif /* __CC_ARM */
/*
主机端口中断 (Host port interrupt)控制器设置此位指示OTG_FS控制器的某个端口状态发生改变。
应用程序需要通过读主机端口控制和状态寄存器(OTG_FS_HPRT)来获得导致此中断的具体信息。
应用程序必需通过清除主机端口控制和状态寄存器的相应位来清除此位。
*/
static uint32_t USB_OTG_USBH_handle_port_ISR (USB_OTG_CORE_HANDLE *pdev)
{
...
#endif /* __CC_ARM */
/*
接收FIFO非空 (RxFIFO non-empty)指示接收FIFO中至少有个一个数据包等待处理。
*/
static uint32_t USB_OTG_USBH_handle_rx_qlvl_ISR (USB_OTG_CORE_HANDLE *pdev)
{
...
#endif /* __CC_ARM */
/*
未完成的同步OUT传输 (Incomplete isochronous OUT transfer)
*/
static uint32_t USB_OTG_USBH_handle_IncompletePeriodicXfer_ISR (USB_OTG_CORE_HANDLE *pdev)
{
